...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/windows-build-tools@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
In order to access the Windows Build Tools binaries installed in
...
<ul>
<li><code>xpack-windows-build-tools-{customField('xpackVersion')}-win32-x64.zip</code></li>
</ul>
